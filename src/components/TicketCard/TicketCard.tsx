...
const _TicketCard: React.FC<TicketCardProps> = ({ eventId, ticket }) => {
  ...
  useEffect(() => {
    ...
    socket
      .off("joined-event-with-ticket", onEventJoined)
      .on("joined-event-with-ticket", onEventJoined);

    return () => {
      socket.off("joined-event-with-ticket");
    };
  }, []);

  const onEventJoined = (data: { event: Partial<Event>; ticketId: string }) => {
    console.log("JOINED EVENT - SOCKET RESPONSE");
    if (data.event.id === eventId && data.ticketId === ticket?.id) {
      setSuccess(true);
      // Imposta l'utente come partecipante
      setUserAsParticipant();
    }
  };

  const setUserAsParticipant = () => {
    // TODO GESTOINE MULTIBIGLIETTI
    cache.modify({
      id: cache.identify({
        __typename: "Event",
        id: eventId,
...
export const TicketCard = React.memo(_TicketCard, (prev, next) => {
  return (
